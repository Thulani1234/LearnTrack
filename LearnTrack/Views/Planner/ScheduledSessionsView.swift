...
struct ScheduledSessionsView: View {
    ...
    private var dailySessions: [StudySession] {
        data.scheduledSessions.filter { Calendar.current.isDate($0.date, inSameDayAs: date) }
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 20) {
            ...
            if dailySessions.isEmpty && calendarEvents.isEmpty {
                VStack(spacing: 16) {
                    ...
                    Text("Create your first study planner and let AI generate a schedule for today.")
                        .font(.system(size: 14))
                        .foregroundColor(AppColors.textSecondary)
                        .multilineTextAlignment(.center)
                        .frame(maxWidth: 260)
                    Button(action: {
                        router.navigate(to: .planSetup)
                    }) {
                        Text("Build My Study Planner")
                            .font(.system(size: 14, weight: .bold))
        ...
        .onReceive(calendarManager.$isAuthorized) { _ in loadCalendarEvents() }
    }
